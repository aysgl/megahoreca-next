...
// const fetcherPro = async (url) => {
//     const res = await fetch(url)
//     const prodata = await res.json()

//     if (res.status !== 200) {
//         throw new Error(prodata.message)
//     }
//     return prodata
// }
...
export default function CategoryDetails() {
    ...
    // const { data: prodata, error: proerror } = useSWR(
    //     () => query.id && `/api/product/${query.id}`,
    //     fetcherPro
    // )

    if (caterror) return <div>{caterror.message}</div>
    if (!catdata) return <div>Loading data cat data...</div>

    // if (proerror) return <div>{proerror.message}</div>
    // if (!prodata) return <div>Loading data cat data...</div>
        ...
        <Container>
            <div className='mb-3'>
                <Row className='d-flex align-items-center'>
                    <Col md={4}>
                        <p className='mb-0 fw-bold'>#{catdata.id} | {catdata.title} |
                            {catdata.subcategory.title}
                        </p>
                    </Col>
